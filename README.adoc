...
Now, we want to check that the last `+lengthBaseTen[gpf]+` digits of the numbers one more than each index are equal to the corresponding GPF itself. To do that, we can just raise ten to the power of the lengths, use it as a modulo on the positive integers, and then check for equality with `+gpf+`. However, there is no sequence for the number ten. There is a sequence for the number one, https://oeis.org/A000012[`+12+`], and we have addition.
...
Now we can combine all the pieces. We take the positive integers modulo (ten to the power of the length base ten of the GPFs), check that they're equal to the GPFs, and lastly filter the positive integers by that boolean mask. The only missing puzzle piece is https://oeis.org/A000027[`+27+`], the sequence of positive integers.

----
fromOne:27
fromOne{gpf=fromOne%(ten^lengthBaseTen[gpf])}
----

Lastly, a little bit of golfing: `+27{27%(b+d:c+c:b+b:12+12)^55642.g=g:6530[23443]}+` (48 characters).
